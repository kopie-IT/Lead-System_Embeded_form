...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Structured items: array of ['description' => ..., 'qty' => ..., 'price' => ..., 'total' => ...]
    ...
    $q_number = 'QT-' . date('Ymd') . '-' . rand(1000, 9999);
    $itemsJson = json_encode($items);
    
    $stmt = $pdo->prepare("INSERT INTO quotations (leads_profile_id, quotation_number, amount, status, items) VALUES (?, ?, ?, ?, ?)");
    $stmt->execute([$profile_id, $q_number, $final_amount, $status, $itemsJson]);
...
<div class="mb-8">
    ...
    <?php else: ?>
    <a href="<?= BASE_PATH ?>/quotations.php" class="text-secondary hover:underline flex items-center gap-1 text-sm">
        <span class="material-symbols-outlined text-sm">arrow_back</span> Back to Quotations
    </a>
    <?php endif; ?>
</div>

<div class="bg-white shadow-sm border border-outline-variant rounded-lg p-6 max-w-5xl mx-auto">
    <form method="POST" id="quotation-form" class="space-y-8">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <label class="block text-sm font-medium text-on-surface mb-1">Quotation Status</label>
                ...
                <table class="w-full text-left border-collapse" id="items-table">
                    ...
                    <tbody id="items-body">
                        <tr class="item-row border-b border-outline-variant/30">
                            <td class="px-2 py-4">
                                <input type="text" name="item_desc[]" placeholder="e.g. Takaful Hibah Plan A" class="w-full bg-white border border-outline-variant rounded-lg p-2 text-sm text-on-surface focus:border-[#005abe] transition-colors" required>
                            </td>
                            <td class="px-2 py-4">
