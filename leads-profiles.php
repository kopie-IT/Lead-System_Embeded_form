...
$page_title = 'Lead Profiles';
require_once __DIR__ . '/includes/header.php';
require_once __DIR__ . '/includes/sidebar.php';

// Fetch all lead profiles
$stmt = $pdo->query("SELECT p.*, COUNT(l.id) as inquiry_count FROM leads_profile p LEFT JOIN leads l ON p.id = l.leads_profile_id GROUP BY p.id ORDER BY p.created_at DESC");
...
<div class="mb-8 flex justify-between items-center">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-3xl font-bold text-black tracking-tight">Leads Profiles</h1>
            <p class="text-black/60 text-sm font-medium">Manage clients and generate quotations/invoices</p>
        </div>
    </div>
...
<div class="bg-white border border-[#005abe]/20 shadow-sm rounded-2xl overflow-hidden mb-12 transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
    <div class="bg-[#005abe]/5 px-6 py-5 border-b border-[#005abe]/10 flex justify-between items-center">
        <h2 class="text-lg font-bold text-black flex items-center gap-2">
            <span class="material-symbols-outlined text-black">group</span> Client Directory
        </h2>
    </div>
    <div class="p-0 overflow-x-auto">
        <table class="w-full text-left text-sm text-black">
            <thead class="bg-[#005abe]/5 text-black font-bold text-[10px] uppercase tracking-wider">
                <tr>
                    <th class="px-6 py-4 border-b border-[#005abe]/10">Profile Created</th>
                    <th class="px-6 py-4 border-b border-[#005abe]/10">Name</th>
                    <th class="px-6 py-4 border-b border-[#005abe]/10">Contact Info</th>
                    <th class="px-6 py-4 border-b border-[#005abe]/10">History</th>
                ...
                <?php if (count($profiles) > 0): ?>
                    <?php foreach ($profiles as $profile): ?>
                        <tr class="group hover:bg-[#005abe]/5 transition-all duration-200">
                            <td class="px-6 py-4 whitespace-nowrap font-medium text-black/80 group-hover:text-black transition-colors"><?= htmlspecialchars(date('M d, Y', strtotime($profile->created_at))) ?></td>
                            <td class="px-6 py-4 font-bold text-black group-hover:translate-x-1 transition-transform"><?= htmlspecialchars($profile->full_name) ?></td>
                            <td class="px-6 py-4">
                                <div class="text-black/60 text-xs mb-0.5"><span class="font-bold text-black/70 uppercase text-[9px] tracking-wider">Phone:</span> <span class="font-mono text-black">+<?= htmlspecialchars($profile->phone_number) ?></span></div>
                                <div class="text-black/60 text-xs"><span class="font-bold text-black/70 uppercase text-[9px] tracking-wider">Email:</span> <a href="mailto:<?= htmlspecialchars($profile->email_address) ?>" class="hover:underline text-black"><?= htmlspecialchars($profile->email_address) ?></a></div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center justify-center px-3 py-1 bg-[#005abe] text-white text-[10px] rounded-full font-bold uppercase tracking-wider group-hover:scale-105 transition-transform border border-[#005abe]/20 shadow-sm">
                                    <?= $profile->inquiry_count ?> Inquiries
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <a href="<?= BASE_PATH ?>/view-profile.php?id=<?= $profile->id ?>" class="inline-flex text-black hover:text-white font-bold text-xs items-center gap-1 bg-white hover:bg-[#005abe] px-4 py-2 rounded-lg transition-all border border-[#005abe]/30 active:scale-95 shadow-sm">
    ...
    <div id="lead-modal-card"
        ...
        <div class="bg-[#005abe] px-6 py-4 flex items-center justify-between rounded-t-2xl flex-shrink-0">
            <div class="flex items-center gap-3">
                ...
                <h3 class="text-lg font-bold text-white">Create Profile / Lead</h3>
            </div>
        ...
        <div class="p-6 overflow-y-auto flex-1 space-y-4">
            ...
            <!-- Profile Only toggle — shown at top for clarity -->
            <div class="flex items-center gap-3 p-3 bg-[#005abe]/5 rounded-xl border border-[#005abe]/20">
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" id="ml-profile-only" class="sr-only peer" onchange="toggleProfileOnly(this.checked)">
                ...
                <div>
                    <p class="text-sm font-semibold text-[#005abe]">Profile Only</p>
                    <p class="text-xs text-black/50">Create contact profile without an inquiry record</p>
                </div>
            </div>
            ...
            <!-- Inquiry-only fields -->
            <div id="inq-fields" class="space-y-4">
                <div>
        ...
        <div class="px-6 py-4 border-t border-gray-100 flex justify-end gap-3 flex-shrink-0 bg-gray-50/60 rounded-b-2xl">
            ...
            <button id="ml-submit-btn" onclick="submitLeadModal()"
                ...
                <span id="ml-btn-label">Create Lead</span>
            </button>
        </div>
    </div>
</div>
...
function openLeadModal() {
    ...
    document.getElementById('ml-profile-only').checked  = false;
    document.getElementById('ml-error').classList.add('hidden');
    toggleProfileOnly(false);
    var btn = document.getElementById('ml-submit-btn');
    btn.disabled = false;
    btn.innerHTML = '<span class="material-symbols-outlined text-sm">save</span> <span id="ml-btn-label">Create Lead</span>';
...
function closeLeadModal() {
    ...
    setTimeout(function () { overlay.classList.add('opacity-0','pointer-events-none'); }, 250);
}
...
function toggleProfileOnly(checked) {
    document.getElementById('inq-type-wrap').classList.toggle('opacity-40', checked);
    document.getElementById('inq-type-wrap').querySelectorAll('select,input').forEach(function(el){ el.disabled = checked; });
    document.getElementById('inq-fields').classList.toggle('hidden', checked);
    var lbl = document.getElementById('ml-btn-label');
    if (lbl) lbl.textContent = checked ? 'Create Profile' : 'Create Lead';
}
...
function submitLeadModal() {
    ...
    var profileOnly = document.getElementById('ml-profile-only').checked ? '1' : '0';
    ...
    fd.append('profile_only',   profileOnly);
    ...
    fetch(BASE + '/create-lead-ajax.php', { method: 'POST', body: fd })
        ...
        .then(function (data) {
            if (data.success) {
                ...
                btn.disabled  = false;
                btn.innerHTML = '<span class="material-symbols-outlined text-sm">save</span> <span id="ml-btn-label">' + (profileOnly === '1' ? 'Create Profile' : 'Create Lead') + '</span>';
